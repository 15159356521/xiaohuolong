...
import { pubUpVideo } from '../../api/upvideo'
...
function MyEditor(props) {
   ...
   /*  useEffect(() => {
    ...
    editorConfig.MENU_CONF['uploadImage'] = {
        server: '/common.upload/uploadImage',
        fieldName: 'file',
       ...
       /*  meta: {
        ...
        customInsert(res, insertFn) {  // 自定义上传视频
            console.log(insertFn, 'customInsert')
            const {code, data, msg} = res

            // 从 res 中找到 url alt href ，然后插入图片
            insertFn(baseIMgURL+data.url,"width:100%;height:100%")
        },
        onInsertedImage(res, insertFn) {  // 自定义上传图片
        //  设置图片尺寸为50%
          insertFn('http://www.wangeditor.com/imgs/logo.jpg', '50%')

        }
    }
     editorConfig.MENU_CONF['uploadVideo'] = {
        // 重写上传路劲
         async customUpload(editor, insertFn) {
            console.log(editor, 'customUploadVideo')
            // 创建一个加载中的提示 
                uploader= tcVod.upload({
                    mediaFile: editor,
                });
                uploader.on('media_progress', function (progressData) {
                    console.log(progressData, 'progressData');
                   
                    message.open({
                        type: 'loading',
                        content: `上传中${((progressData.percent)*100).toFixed(0)}%`,
                        key: 'updatable',
                        duration: 0,
