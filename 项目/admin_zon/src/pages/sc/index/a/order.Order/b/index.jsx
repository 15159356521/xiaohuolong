...
import { getAllList, getUp, PostUp, PostDel } from "@/api/Order";
...
export default function Index() {
  ...
  const getAllRoleGroupData = async () => {
    setLoading(true);
    const { data:{data} } = await getAllList({ status: 1 });
    console.log(data);
  ...
  const onFinish = async (value) => {
    ...
    setSearch(value);
    const up = { ...value, page: 1, limit: 10, status: 1 };
  ...
  const onReset = async () => {
    ...
    const { data:{data,code} } = await getAllList({ status: 1 });
    console.log(data);
    if (code === 200) {
    ...
    <div className={styles.root}>
      <Card>
        <Breadcrumb separator=">" routes style={{ marginBottom: "20px" }}>
          ...
          <Breadcrumb.Item>已发货订单</Breadcrumb.Item>
        </Breadcrumb>
