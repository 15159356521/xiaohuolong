...
export default function Index() {
  ...
  const handleOk = async () => {
    ...
    files.children = shopData;
    setUpLogin(true);
    try {
      console.log(files, "sdfsf");
      // const {
      //   data: { code, msg },
      // } = await PostUp({
      //   ...files,
  ...
  const shopOk = async () => {
    try {
      ...
      fileds.status = commodityData.status 
      fileds.is_points_discount = commodityData.is_points_discount 
      fileds.shop_num = fileds.sort;
      console.log(fileds
        ,fileds.id);
      fileds.id=commodityData.id?commodityData.id:shopData.length+1
    
      // fileds.goods_id=commodityData.id?commodityData.id:formData.goods_i
      let arr = shopData;
      let flag = false;
      console.log(fileds
        ,fileds.id);
      if(shopData.length==0){
        ...
        setShopData((pre) => [...pre]);
          ...
          for(let i=0;i<shopData.length;i++){
            console.log(shopData[i].id,fileds.id);
        if(shopData[i].id==fileds.id){
        flag=true;
        console.log(787877);
          arr[i]=fileds;
          setShopData((pre) => [...pre]);
          ...
          <Space size="middle">
            <Button
              onClick={async () => {
                ...
                setCommodityData(record);
                console.log(record,"wwwwwwwwwwwwwwwwwwwwwwwww");
                commodityForm.setFieldsValue({ ...record });
                getImg(record);
              }}
            >
              编辑
  ...
  const Remove = (file, item) => {
    ...
    if (item.url) {
      ...
      return file.filter(
        (item1) =>
          item1 !==
          "https://l.src.xiaohuolongfujiankeji.com" + item.response.data.url
      );
    }
  };
  // 拖动
                  ...
                  >
                    ...
                    <Form.Item label="积分折扣" name="is_points_discount">
                      <Switch
                        ...
                        checked={
                          commodityData.is_points_discount == "允许"
                            ? true
                            : false
                        }
                        onChange={async(e) => {
                          ...
                          console.log(
                            commodityData.is_points_discount === "不允许"
                          );
                          console.log(e, "asfsdf大大大大大大");
                          e
                            ? setCommodityData((preState) => {
                                return {
                                  ...preState,
                                  is_points_discount: "允许",
                                };
                              })
                            : setCommodityData((preState) => {
                                return {
                                  ...preState,
                                  is_points_discount: "不允许",
                                };
                              });
                        }}
                      />
